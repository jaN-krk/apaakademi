'use client';
// React Bits / David Haz. See docs/react-bits-LICENSE.md.
...
const TextLoop = ({
  ...
  useEffect(() => {
    ...
    if (prefersReduced || paused || speed <= 0) return undefined;
    ...
    let visible = false;
    let hovering = false;
    const syncPlayback = () => { if(visible && !document.hidden && !hovering) tween.resume(); else tween.pause(); };
    const observer = new IntersectionObserver(([entry]) => {visible=entry.isIntersecting;syncPlayback();});
    if(root) observer.observe(root);
    document.addEventListener('visibilitychange',syncPlayback);
    tween.pause();
    const pause = () => {hovering=true;syncPlayback();};
    const resume = () => {hovering=false;syncPlayback();};
  ...
  }, [metrics, speed, direction, pauseOnHover, paused]);
